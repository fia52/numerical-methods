...
void Jacoby(double[][n], double[], double[], double[], int);
...
int main()
{
	...
	double eps1, criteria = one, tmp, normb;
	//
	for (i = 0; i < n; i++) 
	{
		c[i] = zero;
	...
	//
	cout << "THE Yakobi METHOD" << endl << endl << endl;
	...
	//
	if (normb > one || normb <= half) eps1 = eps;
	else eps1 = eps * (one - normb) / normb;
	//
	cout << endl << "SOLUTION OF SLAE using the Jacoby metod with eps = " << eps1 << endl << endl;
	do {
		Jacoby(B, x, c, xtmp, nsz);
	...
	} while ((criteria > eps1 || criteria == zero) && k <= MAXITER);
	//
...
double norm(double B[][n], int m)
{
	...
	{
		sum = zero;
		for (j = 0; j < m; ++j)sum = sum + fabs(B[i][j]);
		tmp[i] = sum;
	}
	fnorm = *tmp, tmp + m;
	delete[] tmp;
...
void Jacoby(double mb[][n], double vx[], double vc[], double vxtmp[], int sz)
{
	int i, j;
	double sum;
	for (i = 0; i < sz; i++)
